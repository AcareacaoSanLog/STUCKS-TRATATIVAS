(function () {
  ...
  var BASE_COLUMNS = IMPORT_FIELDS.concat(['cep', 'cidade_cep', 'bairro', 'status_cep', 'avaria', 'prioridade']);
  ...
  var FIELD_ALIASES = {
    shipment_id: ['shipment_id', 'shipment id', 'br', 'tn', 'tracking number', 'tracking_number', 'sls tracking number', 'slstrackingnumber', 'rastreio', 'codigo rastreio', 'codigorastreio', 'pedido', 'orderid', 'order id'],
    tracking_status: ['tracking_status', 'tracking status', 'status', 'status tracking', 'last status', 'ultimo status'],
    ageing_last_status: ['ageing_last_status', 'aging_last_status', 'ageing last status', 'aging last status', 'ageing', 'aging', 'dias parados', 'diasparados', 'days', 'dias', 'idade status', 'idade do status'],
    cogs: ['cogs', 'cog', 'valor', 'cost', 'custo', 'cogs r$', 'cod amount', 'codamount', 'amount', 'original asf', 'originalasf'],
    buyer_city: ['buyer_city', 'buyer city', 'cidade', 'cidade comprador', 'buyer cidade', 'city'],
    driver_id: ['driver_id', 'driver id', 'id driver', 'id motorista', 'motorista id', 'id entregador', 'id do entregador'],
    driver_name: ['driver_name', 'driver name', 'nome driver', 'nome motorista', 'motorista', 'entregador', 'driver', 'nome entregador', 'nome do entregador'],
    tratativa: ['tratativa', 'tratativas', 'palavra chave', 'palavrachave', 'palavra-chave', 'keyword', 'observacao', 'observação', 'obs', 'motivo'],
    avaria: ['avaria', 'damaged tag', 'damagedtag', 'damage', 'danificado'],
    cep: ['cep', 'zipcode', 'zip code', 'postalcode', 'postal code']
  };
  ...
  var IMPORT_COMPAT_VERSION = 'stucks-import-tn-status-dias-valor-cidade-entregador-v2';
...
var MONITOR_DEFS = {
    received: { label: 'Received', title: '📥 Received', statusKeys: ['hubReceived'], rawKeys: ['hubreceived', 'socreceived'] },
    assigned: { label: 'Assigned', title: '🏷️ Assigned', statusKeys: ['hubAssigned'], rawKeys: ['hubassigned'] },
    soclh: { label: 'SOC LH', title: '🚚 SOC LH', statusKeys: ['socLHTransported'], rawKeys: ['soclhtransported', 'soclhtransport'] },
    onhold: { label: 'OnHold', title: '⛔ OnHold', statusKeys: ['onhold'], rawKeys: ['onhold', 'onhol'] }
  };
  ...
  function cacheElements() {
    ...
    els.monitorTotalLabel = document.getElementById('monitorTotalLabel');
    els.monitorTotalValue = document.getElementById('monitorTotalValue');
    els.monitorTotalNote = document.getElementById('monitorTotalNote');
    els.monitorCityValue = document.getElementById('monitorCityValue');
    els.monitorNeighborhoodValue = document.getElementById('monitorNeighborhoodValue');
    els.monitorSummaryTitle = document.getElementById('monitorSummaryTitle');
    els.monitorCasesTitle = document.getElementById('monitorCasesTitle');
    els.monitorSummaryTable = document.getElementById('monitorSummaryTable');
    els.monitorCasesTable = document.getElementById('monitorCasesTable');
  }
  ...
  function bindEvents() {
    ...
    if (els.syncSheetTreatmentsBtn) els.syncSheetTreatmentsBtn.addEventListener('click', function () { flushTreatmentSheetQueue({ forceAll: true }); });
  ...
  function loadDamageList() {
    var raw = localStorage.getItem(DAMAGE_STORAGE_KEY) || '';
  ...
  function loadTreatmentMap() {
    ...
    try {
      var data = JSON.parse(localStorage.getItem(TREATMENT_STORAGE_KEY) || '{}');
  ...
  function persistTreatmentMap() {
    ...
    localStorage.setItem(TREATMENT_STORAGE_KEY, JSON.stringify(data));
  ...
  function loadTreatmentSheetConfig() {
    ...
    treatmentSheetSync.fileName = cleanCell(localStorage.getItem(TREATMENT_SHEET_URL_KEY) || '');
    if (els.treatmentSheetUrlInput) els.treatmentSheetUrlInput.value = treatmentSheetSync.fileName || 'Estoque local do navegador ativo';
    updateTreatmentSheetStatus('Tratativas salvas localmente no navegador. Use “Baixar Estoque Excel” para gerar o arquivo Estoque de tratativas.xlsx.', 'ok');
  }

  async function saveTreatmentSheetConfig() {
    await connectLocalTreatmentWorkbook();
  }
  ...
  async function connectLocalTreatmentWorkbook() {
    try {
      if (!window.XLSX) throw new Error('Biblioteca XLSX não carregada.');
      if (!window.showSaveFilePicker) {
        downloadTreatmentWorkbook();
        updateTreatmentSheetStatus('Seu navegador não permite conectar arquivo local direto. Baixei o Estoque de tratativas.xlsx como backup.', 'warn');
        return;
      }
      ...
      localStorage.setItem(TREATMENT_SHEET_URL_KEY, treatmentSheetSync.fileName);
      if (els.treatmentSheetUrlInput) els.treatmentSheetUrlInput.value = treatmentSheetSync.fileName;
      await saveTreatmentsToConnectedFile({ silent: false });
      updateTreatmentSheetStatus('Arquivo local conectado. Ao clicar em salvar tratativas, o Excel conectado será atualizado.', 'ok');
    } catch (error) {
      if (error && error.name === 'AbortError') return;
      console.error(error);
      updateTreatmentSheetStatus('Não consegui conectar o arquivo local. As tratativas continuam salvas no navegador. Erro: ' + (error.message || error), 'error');
    }
  }
  ...
  async function saveTreatmentsToConnectedFile(options) {
    ...
    if (!options.silent) updateTreatmentSheetStatus('Estoque de tratativas.xlsx atualizado localmente.', 'ok');
    return true;
  }
  ...
  async function flushTreatmentSheetQueue(options) {
    ...
    try {
      if (treatmentSheetSync.fileHandle) {
        ...
        updateTreatmentSheetStatus('Estoque de tratativas.xlsx atualizado: ' + treatmentRowsForTable().length + ' BR(s) com tratativa.', 'ok');
      } else {
        if (!options.silent) {
          downloadTreatmentWorkbook();
          updateTreatmentSheetStatus('Estoque de tratativas.xlsx baixado com ' + treatmentRowsForTable().length + ' BR(s).', 'ok');
        } else {
          updateTreatmentSheetStatus('Tratativas salvas no estoque local do navegador: ' + treatmentRowsForTable().length + ' BR(s). Para backup em Excel, clique em “Baixar Estoque Excel”.', 'ok');
        }
      }
    } catch (error) {
      records.forEach(function (record) { treatmentSheetSync.pending.set(record.shipment_id, record); });
      updateTreatmentSheetStatus('Tratativas salvas no navegador, mas não consegui atualizar o Excel conectado. Erro: ' + (error.message || error), 'error');
    } finally {
  ...
  function applyDamageListFromInput() {
    ...
    localStorage.setItem(DAMAGE_STORAGE_KEY, list.join('\n'));
    if (els.damageShipmentInput) els.damageShipmentInput.value = '';
    if (els.damageSingleInput) els.damageSingleInput.value = '';
  ...
  function clearDamageList() {
    ...
    localStorage.removeItem(DAMAGE_STORAGE_KEY);
    if (els.damageShipmentInput) els.damageShipmentInput.value = '';
    if (els.damageSingleInput) els.damageSingleInput.value = '';
    if (els.damageNoteInput) els.damageNoteInput.value = '';
    applyDamageListToRows();
  ...
  function saveSingleTreatmentFromForm() {
    ...
    setStatus('Tratativa salva para a BR ' + br + '.', 'ok');
  }
  ...
  function applyTreatmentsImport(rawRows) {
  ...
  function removeDamageShipment(br) {
    ...
    localStorage.setItem(DAMAGE_STORAGE_KEY, Array.from(damageShipmentSet).sort(localeSort).join('\n'));
    applyDamageListToRows();
    populateFilters();
    applyAndRender();
  ...
  async function processImportedRows(rawRows, fileName) {
    ...
    if (missing.length) throw new Error('Colunas obrigatórias não encontradas: ' + missing.map(label).join(', ') + '. Esta versão aceita também bases com TN/Status/Dias parados/Valor/Cidade/Entregador.');
    ...
    applyAndRender();
    persistCurrentBaseSilent();
    setStatus('Importação concluída: ' + state.rows.length + ' stucks carregados de ' + fileName + '. Tratativas salvas foram reaplicadas automaticamente pela BR.', 'ok');
  }
  ...
  function applyCepImport(rawRows, fileName) {
    ...
    for (var i = headerIndex + 1; i < rawRows.length; i++) {
      ...
      // A importação de CEP agora também pode trazer o status atualizado.
      // A BR é a chave: se a mesma BR já tinha CEP/status anterior, a nova importação sobrescreve esses dados.
      if (importedCep || importedStatus) {
        cepUpdateMap.set(key, { cep: importedCep, status: importedStatus });
      }
    }
  ...
  function monitorSearchMatches(rows) {
    ...
    if (!search) return [];
    return rows.filter(function (row) {
      var haystack = normalizeSearch([row.shipment_id, row.tracking_status, row.buyer_city, row.cidade_cep, row.bairro, row.driver_name, row.driver_id, row.tratativa, row.cep].join(' '));
  ...
  function renderMonitoring() {
    ...
    if (els.monitorCasesTitle) els.monitorCasesTitle.textContent = '🔎 BRs em ' + def.label;
  ...
  function groupMonitorRows(rows) {
    ...
    rows.forEach(function (row) {
      ...
      if (!map.has(key)) map.set(key, { city: city, neighborhood: neighborhood, ageing: row._ageing, ageingKey: ageingKey, rows: [], treatments: new Map() });
  ...
  function renderMonitorSummaryTable(groups) {
    ...
    var head = '<thead><tr><th>Cidade</th><th>Bairro</th><th>Tratativa</th><th>Dias parados</th><th>Qtd</th><th></th></tr></thead>';
    var body = groups.length ? groups.map(function (item, index) {
      ...
      '</tr>';
    }).join('') : '<tr><td colspan="6">Nenhuma BR encontrada neste status.</td></tr>';
  ...
  function monitorModalFilteredRows() {
    ...
    var rows = monitorModalAllRows.filter(function (row) {
      ...
      if (search) {
        var haystack = normalizeSearch([row.shipment_id, row.tracking_status, row.cep, monitorRowCity(row), monitorRowNeighborhood(row), row.driver_id, row.driver_name, row.tratativa].join(' '));
        if (haystack.indexOf(search) < 0) return false;
      }
      return true;
    });
  ...
  function saveMonitorModalTreatments() {
    if (!els.monitorModalTable) return;
    ...
    populateMonitorModalFilters();
    renderMonitorModalTable();
    setStatus('Alterações salvas para ' + monitorModalAllRows.length + ' BR(s) do grupo.', 'ok');
  }
  ...
  function groupForExport(primary, secondary) {
    return topGroup(state.filtered, primary, secondary).map(function (item) {
      return {
        ...
        cogs: item.rows.reduce(function (sum, row) { return sum + row._cogsNumber; }, 0)
      };
    });
  }
  ...
  function clearHistory() { localStorage.removeItem(HISTORY_KEY); renderHistory(); setStatus('Histórico de importações limpo.', 'warn'); }
  ...
  function exportRows(rows, fileName) {
    ...
    if (window.XLSX && XLSX.utils && XLSX.writeFile) {
      ...
      setStatus('Arquivo exportado: ' + safeName + '. Dias parados exportado com o valor exato.', 'ok');
      return;
    }
    ...
    setStatus('Arquivo exportado: ' + fallbackName + '. Dias parados exportado com o valor exato.', 'ok');
  }
  ...
  function formatNumber(value) { if (!Number.isFinite(value)) return '0'; return value.toLocaleString('pt-BR', { maximumFractionDigits: 2 }); }
