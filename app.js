...
async function saveDashboardStateToCloud(kind,fileName){if(!authClient||!currentSession)throw new Error('Login não encontrado para salvar a base na nuvem.'); const payload=buildCloudPayload(kind,fileName); const rec={id:CLOUD_STATE_ID,payload,row_count:state.rows.length,cep_count:state.cepMap.size,current_file:state.currentFile||fileName||'',imported_at:state.importedAt,updated_by:currentSession.user?.id||null,updated_by_email:currentSession.user?.email||'',updated_at:new Date().toISOString()}; const {error}=await authClient.from(CLOUD_STATE_TABLE).upsert(rec,{onConflict:'id'}); if(error)throw new Error('Supabase cloud state: '+error.message); return rec;}
async function loadDashboardStateFromCloud(silent){if(!authClient||!currentSession)return false; const {data,error}=await authClient.from(CLOUD_STATE_TABLE).select('payload,row_count,cep_count,current_file,imported_at,updated_at,updated_by_email').eq('id',CLOUD_STATE_ID).maybeSingle(); if(error){console.warn('loadDashboardStateFromCloud',error); const msg=error.code==='42P01'?'Tabela de base na nuvem ainda não existe. Rode o SQL enviado no Supabase.':'Erro ao carregar base da nuvem: '+error.message; if(!silent||error.code==='42P01')setStatus(msg,'warn'); return false;} const payload=data?.payload; if(!payload||!Array.isArray(payload.rows)||!payload.rows.length)return false; applyCloudPayload(payload); const when=data.updated_at?formatDate(data.updated_at):'-'; if(!silent)setStatus('Base carregada da nuvem: '+state.rows.length+' stucks. Última atualização: '+when+'.','ok'); else setStatus('Base carregada da nuvem: '+state.rows.length+' stucks. Última atualização: '+when+'.','ok'); return true;}
