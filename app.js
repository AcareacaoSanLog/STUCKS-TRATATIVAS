...
function bindMonitorDayFilter(){const btn=document.getElementById('mDaysButton'), menu=document.getElementById('mDaysMenu'), all=document.getElementById('mDaysAll'); if(btn&&menu)btn.addEventListener('click',()=>{const open=!menu.classList.contains('open'); menu.classList.toggle('open',open); btn.setAttribute('aria-expanded',open?'true':'false');}); if(all)all.addEventListener('change',()=>{if(all.checked)document.querySelectorAll('.mDaysCheck').forEach(c=>c.checked=false); readMonitorFilters(); renderMonitor();}); document.querySelectorAll('.mDaysCheck').forEach(c=>c.addEventListener('change',()=>{if(c.checked&&all)all.checked=false; const checked=[...document.querySelectorAll('.mDaysCheck:checked')]; if(!checked.length&&all)all.checked=true; readMonitorFilters(); renderMonitor();}));}
