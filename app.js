 ...
 el.content.innerHTML=
  ...
  `<div class="old-main-grid">
    ...
    <section class="panel">
      ...
      <div class="table-wrap">${table(rows.slice().sort((a,b)=>(b.ageing_num||0)-(a.ageing_num||0)).slice(0,8),['shipment_id','tracking_status','ageing_last_status','city','bairro','driver','keyword','tratativa_display'],{tracking_status:r=>statusPill(r.tracking_status),keyword:r=>pill(displayKeyword(r)),tratativa_display:r=>esc(r.tratativa_display)})}</div>
    </section>
    <section class="panel">
...
function renderBase(){el.title.textContent='📦 Base Stucks'; const rows=filteredRows(); el.content.innerHTML=filtersHtml()+`<div class="table-wrap">${table(rows,['shipment_id','tracking_status','ageing_last_status','cogs','city','bairro','driver','keyword','avaria','tratativa_display'],{cogs:r=>formatMoney(r.cogs_num),keyword:r=>pill(displayKeyword(r)),avaria:r=>pill(r.avaria==='Sim'?'Sim':'Não'),tracking_status:r=>statusPill(r.tracking_status),tratativa_display:r=>esc(r.tratativa_display)})}</div>`; bindFilterInputs();}
