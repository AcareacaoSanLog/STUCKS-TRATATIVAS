...
function onClick(e){
 ...
 if(a==='sync-cloud') syncAllTreatments();
 if(a==='pull-cloud') loadTreatmentsFromCloud(false);
 if(a==='save-one-treatment') saveOneTreatment();
 if(a==='remove-treatment') removeTreatment(b.dataset.br);
 if(a==='add-damages') addDamages();
 if(a==='clear-damages') clearDamages();
 if(a==='copy-brs') copyOpenBRs();
 if(a==='export-modal') exportRows(window.__modalRows||[],'detalhes_stucks.xlsx');
}
...
function saveLocal(silent){localStorage.setItem(STORE,JSON.stringify({rows:state.rows,currentFile:state.currentFile,importedAt:state.importedAt,treatments:[...state.treatments],history:state.history})); if(!silent)setStatus('Base salva neste navegador.','ok');}
...
function saveCep(){localStorage.setItem(STORE_CEP,JSON.stringify([...state.cepMap]));}
function saveDamages(){localStorage.setItem(STORE_DAMAGES,JSON.stringify([...state.damages]));}
...
function addHistory(file,count){state.history.unshift({file,count,at:new Date().toISOString()}); state.history=state.history.slice(0,20); localStorage.setItem(STORE_HISTORY,JSON.stringify(state.history));}

function table(rows,fields,fmt={}){return `<table><thead><tr>${fields.map(f=>`<th>${label(f)}</th>`).join('')}</tr></thead><tbody>${rows.length?rows.map(r=>`<tr>${fields.map(f=>`<td>${fmt[f]?fmt[f](r):esc(r[f]??'-')}</td>`).join('')}</tr>`).join(''):`<tr><td colspan="${fields.length}">Nenhum dado.</td></tr>`}</tbody></table>`;}
...
function unique(vals,num){let a=[...new Set(vals.filter(v=>v!==''&&v!=null))]; if(num)a.sort((x,y)=>Number(x)-Number(y)); else a.sort((a,b)=>String(a).localeCompare(String(b),'pt-BR')); return a;}
