...
const CLOUD_STATE_TABLE='stucks_cloud_state';
const CLOUD_STATE_ID='stucks_latest_dataset';
...
const FIELD_ALIASES={
 ...
 keyword:['palavra chave','palavrachave','palavra-chave','palavra_chave','keyword','key word','possui palavra chave','tem palavra chave'],
 tratativa:['tratativa','tratativas','observacao','observação','obs','motivo','tratamento','comentario','comentário'],
 ...
 bairro:['bairro','neighborhood','district','localidade','bairro cep','bairro_cep','bairro entrega','bairro do cep','bairro destino','bairro do cliente','nome bairro','neighborhood name','buyer neighborhood','buyer_neighborhood','buyer address neighborhood','buyer_address_neighborhood','address neighborhood','receiver neighborhood','receiver_neighborhood','customer neighborhood','buyer address district','buyer_address_district','receiver address district','receiver_address_district','destination neighborhood','destination_neighborhood'],
 cidade_cep:['cidade cep','cidade_cep','cidade entrega','cidade do cep','cidade destino','cidade cliente','buyer address city','buyer_address_city','receiver address city','receiver_address_city','destination city','destination_city'],
...
let state={view:'dashboard',rows:[],filtered:[],treatments:new Map(),damages:new Set(),cepMap:new Map(),history:[],filters:{status:'all',statusGroup:'all',city:'all',driver:'all',ageing:'all',priority:'all',avaria:'all',cep:'all',search:''},monitor:{type:'received',city:'all',bairro:'all',days:'all',sort:'asc',search:''},currentFile:'',importedAt:null};
const el={};
let filterTimer=null, monitorTimer=null, monitorDaysMenuOpen=false, monitorCityMenuOpen=false, monitorMenuScroll={city:0,days:0};
...
function boot(){
 ...
 appStarted=true;
 try{
   cache(); bind(); loadLocalState(); render(); hydrateCloudOnBoot();
   setStatus('Dashboard carregado. Buscando base salva na nuvem...','warn');
...
function onClick(e){
 ...
 if(b.id==='saveLocalBtn'){e.preventDefault(); return runAsync(saveDashboardStateManual);}
 ...
 if(a==='copy-monitor-brs') return runAsync(copyMonitorBRs);
...
async function importStucks(raw,fileName){
 ...
 for(let i=headerIndex+1;i<raw.length;i++){
  ...
  row.ageing_num=parseAgeing(row.ageing_last_status); row.cogs_num=parseMoney(row.cogs); row.avaria=normalizeYesNo(row.avaria)||'Não'; row.keyword=normalizeKeyword(row.keyword); row.tratativa=normalizeImportedTreatment(row.tratativa); row.cep=normalizeCep(row.cep); row.cidade_cep=clean(row.cidade_cep); row.bairro=clean(row.bairro); enrich(row); rows.push(row);
 }
 state.rows=dedupe(rows); state.currentFile=fileName; state.importedAt=new Date().toISOString(); applyCep(); applyDamages(); applyTreatments(); addHistory(fileName,state.rows.length); resetFilters(); saveLocal(true); render(); setStatus('Importação concluída: '+state.rows.length+' stucks carregados de '+fileName+'. Salvando base na nuvem...','warn'); try{await saveDashboardStateToCloud('stucks',fileName); setStatus('Importação concluída e salva na nuvem: '+state.rows.length+' stucks carregados de '+fileName+'.','ok');}catch(err){console.error(err); setStatus('Importação concluída, mas não consegui salvar na nuvem: '+(err.message||err),'error');}
}
async function importCep(raw,fileName){
 ...
 for(let i=headerIndex+1;i<raw.length;i++){
  ...
  const item=hydrateCepItem({cep,bairro:clean(map.bairro!=null?r[map.bairro]:''),cidade:cepCityValue(r,map),status:clean(map.tracking_status!=null?r[map.tracking_status]:(map.status_cep!=null?r[map.status_cep]:''))});
  if(br){state.cepMap.set(br,mergeCepRecord(state.cepMap.get(br),item,'BR')); byBr++;}
  if(cep&&(item.bairro||item.cidade||item.status)){state.cepMap.set('CEP:'+cep,mergeCepRecord(state.cepMap.get('CEP:'+cep),item,'CEP')); byCep++;}
 }
 applyCep(); saveCep(); saveLocal(true); render(); setStatus('CEP importado. Salvando cruzamento na nuvem...','warn'); try{await saveDashboardStateToCloud('cep',fileName); setStatus('CEP importado e salvo na nuvem: '+byBr+' BRs cruzadas e '+byCep+' CEPs/bairros mapeados.','ok');}catch(err){console.error(err); setStatus('CEP importado, mas não consegui salvar na nuvem: '+(err.message||err),'error');}
}
...
function hydrateCepItem(item){const base=item.cep?state.cepMap.get('CEP:'+normalizeCep(item.cep)):null; if(base){if(!item.cidade&&base.cidade)item.cidade=base.cidade; if(!item.bairro&&base.bairro&&isTeixeiraCepBase(base))item.bairro=base.bairro; if(!item.status&&base.status)item.status=base.status;} return item;}
function mergeCepRecord(existing,item,tipo){existing=existing||{}; return {cep:item.cep||existing.cep||'',bairro:item.bairro||existing.bairro||'',cidade:item.cidade||existing.cidade||'',status:item.status||existing.status||'',tipo};}
function dedupe(rows){const m=new Map(); rows.forEach(r=>m.set(r.shipment_id,r)); return [...m.values()];}
function enrich(r){r.keyword=normalizeKeyword(r.keyword); r.tratativa=normalizeImportedTreatment(r.tratativa); r.city=clean(r.cidade_cep)||clean(r.buyer_city)||'Sem cidade'; r.bairro=r.bairro||'Sem bairro'; r.driver=clean(r.driver_name)||clean(r.driver_id)||'Sem driver'; r.priority=priority(r); r.status_key=statusKey(r.tracking_status); r.tratativa_display=displayTreatment(r);}
function applyCep(){state.rows.forEach(r=>{const direct=state.cepMap.get(r.shipment_id); if(direct&&direct.cep)r.cep=direct.cep; const byCep=r.cep?state.cepMap.get('CEP:'+normalizeCep(r.cep)):null; const c={cep:direct?.cep||byCep?.cep||'',cidade:direct?.cidade||byCep?.cidade||'',bairro:direct?.bairro||byCep?.bairro||'',status:direct?.status||byCep?.status||''}; if(c.cep||c.cidade||c.bairro||c.status){if(c.cep)r.cep=c.cep; if(c.cidade)r.cidade_cep=c.cidade; if(c.bairro&&isTeixeiraCepMatch(r,c))r.bairro=c.bairro; if(c.status)r.tracking_status=c.status;} enrich(r);});}
function isTeixeiraCepMatch(row,cepData){return [cepData?.cidade,row?.cidade_cep,row?.buyer_city,row?.city].some(v=>normSearch(v).includes('teixeira de freitas'));}
function isTeixeiraCepBase(cepData){return normSearch(cepData?.cidade).includes('teixeira de freitas');}
...
function displayKeyword(r){const k=clean(r.keyword); return k||'Não';}
function treatmentInputDisplay(r){const t=clean(r&&r.tratativa); return t || 'Sem tratativa';}
function normalizeTreatmentSave(v){const t=normalizeImportedTreatment(v); return normSearch(t)==='sem tratativa' ? '' : t;}

function setView(view){state.view=view; if(view.startsWith('monitor-')){const nextType=view.replace('monitor-',''); if(state.monitor.type!==nextType){state.monitor={type:nextType,city:'all',bairro:'all',days:'all',sort:'asc',search:''}; monitorCityMenuOpen=false; monitorDaysMenuOpen=false; monitorMenuScroll={city:0,days:0};} else state.monitor.type=nextType;} render();}
...
function monitorCityValues(){const v=state.monitor.city; return Array.isArray(v)?v:(v&&v!=='all'?[v]:[]);}
function monitorCityLabel(){const vals=monitorCityValues(); if(!vals.length)return 'Todas'; if(vals.length<=2)return vals.join(', '); return vals.length+' cidades selecionadas';}
function isMonitorCitySelected(value){return monitorCityValues().includes(String(value));}
function matchesMonitorCity(row){const vals=monitorCityValues(); return !vals.length||vals.includes(row.city);}
function monitorDayValues(){const v=state.monitor.days; return Array.isArray(v)?v:(v&&v!=='all'?[v]:[]);}
function monitorDayLabel(){const vals=monitorDayValues(); if(!vals.length)return 'Todos'; if(vals.length<=3)return vals.map(v=>v==='Sem dias'?v:v+' dias').join(', '); return vals.length+' dias selecionados';}
function isMonitorDaySelected(value){return monitorDayValues().includes(String(value));}
function matchesMonitorDay(row){const vals=monitorDayValues(); if(!vals.length)return true; return vals.includes(row.ageing_num==null?'Sem dias':String(row.ageing_num));}
function renderMonitor(){rememberMonitorMenuScroll(); const pageX=window.scrollX,pageY=window.scrollY; const def=monitorLabel(state.monitor.type); el.title.textContent='📥 '+def; const rows=state.rows.filter(r=>monitorMatch(r,state.monitor.type)); normalizeMonitorFilters(rows); const visibleRows=monitorVisibleRows(rows); const groups=monitorGroups(rows); window.__groups=groups; el.content.innerHTML=monitorFiltersHtml(rows)+`<div class="panel"><div class="panel-title-row"><div><h3>BRs em ${def} por cidade/bairro</h3><p class="tabs-note">${visibleRows.length} BR(s) na visão atual. Clique em Abrir para preencher ou revisar tratativas.</p></div><button class="btn primary" data-action="copy-monitor-brs">Copiar todas as BRs</button></div><div class="table-wrap"><table class="monitor-table"><thead><tr><th>Cidade</th><th>Bairro</th><th>Status</th><th>Tratativa</th><th>Dias parados</th><th>Qtd</th><th></th></tr></thead><tbody>${groups.length?groups.map((g,i)=>`<tr><td>${esc(g.city)}</td><td>${esc(g.bairro)}</td><td>${esc(g.status)}</td><td class="treatment">${esc(g.treatment)}</td><td><b>${g.daysText}</b></td><td><b>${g.rows.length}</b></td><td><button class="mini" data-action="open-group" data-key="${i}">Abrir</button></td></tr>`).join(''):'<tr><td colspan="7">Nenhuma BR encontrada neste status.</td></tr>'}</tbody></table></div></div>`; bindMonitorFilters(); restoreMonitorMenuScroll(); requestAnimationFrame(()=>window.scrollTo(pageX,pageY));}
function monitorFiltersHtml(rows){const cities=unique(rows.map(r=>r.city)); const selectedCityRows=rows.filter(matchesMonitorCity); const bairros=unique(selectedCityRows.map(r=>r.bairro)); const days=unique(selectedCityRows.filter(r=>state.monitor.bairro==='all'||r.bairro===state.monitor.bairro).map(r=>r.ageing_num==null?'Sem dias':String(r.ageing_num)),true); const cityChecks=cities.map(city=>`<label class="multi-option"><input type="checkbox" class="mCityCheck" value="${esc(city)}" ${isMonitorCitySelected(city)?'checked':''}><span>${esc(city)}</span></label>`).join(''); const dayChecks=days.map(day=>`<label class="multi-option"><input type="checkbox" class="mDaysCheck" value="${esc(day)}" ${isMonitorDaySelected(day)?'checked':''}><span>${esc(day==='Sem dias'?day:day+' dias')}</span></label>`).join(''); return `<div class="filters"><div class="field multi-field"><span>Cidade</span><div class="multi-select" id="mCityMulti"><button class="multi-select-button" id="mCityButton" type="button" aria-expanded="${monitorCityMenuOpen?'true':'false'}">${esc(monitorCityLabel())}</button><div class="multi-select-menu ${monitorCityMenuOpen?'open':''}" id="mCityMenu"><label class="multi-option"><input type="checkbox" id="mCityAll" value="all" ${monitorCityValues().length?'':'checked'}><span>Todas</span></label>${cityChecks}</div></div></div><label>Bairro<select id="mBairro"><option value="all">Todos</option>${opts(bairros,state.monitor.bairro)}</select></label><div class="field multi-field"><span>Dias parados</span><div class="multi-select" id="mDaysMulti"><button class="multi-select-button" id="mDaysButton" type="button" aria-expanded="${monitorDaysMenuOpen?'true':'false'}">${esc(monitorDayLabel())}</button><div class="multi-select-menu ${monitorDaysMenuOpen?'open':''}" id="mDaysMenu"><label class="multi-option"><input type="checkbox" id="mDaysAll" value="all" ${monitorDayValues().length?'':'checked'}><span>Todos</span></label>${dayChecks}</div></div></div><label>Ordenar dias<select id="mSort"><option value="asc">Menor para maior</option><option value="desc" ${state.monitor.sort==='desc'?'selected':''}>Maior para menor</option></select></label><label>Buscar BR<input id="mSearch" value="${esc(state.monitor.search)}" placeholder="Digite BR ou tracking"></label></div>`;}
function bindMonitorFilters(){['mBairro','mSort','mSearch'].forEach(id=>{const x=document.getElementById(id); if(!x)return; x.addEventListener(id==='mSearch'?'input':'change',()=>{readMonitorFilters(); if(id==='mSearch') scheduleMonitorRender(); else renderMonitor();});}); bindMonitorCityFilter(); bindMonitorDayFilter();}
function bindMonitorCityFilter(){const btn=document.getElementById('mCityButton'), menu=document.getElementById('mCityMenu'), all=document.getElementById('mCityAll'); if(btn&&menu)btn.addEventListener('click',()=>{const open=!menu.classList.contains('open'); monitorCityMenuOpen=open; menu.classList.toggle('open',open); btn.setAttribute('aria-expanded',open?'true':'false');}); if(all)all.addEventListener('change',()=>{monitorCityMenuOpen=true; rememberMonitorMenuScroll(); if(all.checked)document.querySelectorAll('.mCityCheck').forEach(c=>c.checked=false); readMonitorFilters(); renderMonitor();}); document.querySelectorAll('.mCityCheck').forEach(c=>c.addEventListener('change',()=>{monitorCityMenuOpen=true; rememberMonitorMenuScroll(); if(c.checked&&all)all.checked=false; const checked=[...document.querySelectorAll('.mCityCheck:checked')]; if(!checked.length&&all)all.checked=true; readMonitorFilters(); renderMonitor();}));}
function bindMonitorDayFilter(){const btn=document.getElementById('mDaysButton'), menu=document.getElementById('mDaysMenu'), all=document.getElementById('mDaysAll'); if(btn&&menu)btn.addEventListener('click',()=>{const open=!menu.classList.contains('open'); monitorDaysMenuOpen=open; menu.classList.toggle('open',open); btn.setAttribute('aria-expanded',open?'true':'false');}); if(all)all.addEventListener('change',()=>{monitorDaysMenuOpen=true; rememberMonitorMenuScroll(); if(all.checked)document.querySelectorAll('.mDaysCheck').forEach(c=>c.checked=false); readMonitorFilters(); renderMonitor();}); document.querySelectorAll('.mDaysCheck').forEach(c=>c.addEventListener('change',()=>{monitorDaysMenuOpen=true; rememberMonitorMenuScroll(); if(c.checked&&all)all.checked=false; const checked=[...document.querySelectorAll('.mDaysCheck:checked')]; if(!checked.length&&all)all.checked=true; readMonitorFilters(); renderMonitor();}));}
function readMonitorFilters(){const cityChecked=[...document.querySelectorAll('.mCityCheck:checked')].map(x=>x.value), cityAll=document.getElementById('mCityAll')?.checked; state.monitor.city=cityAll?'all':(cityChecked.length?cityChecked:'all'); state.monitor.bairro=document.getElementById('mBairro')?.value||'all'; const checked=[...document.querySelectorAll('.mDaysCheck:checked')].map(x=>x.value), allChecked=document.getElementById('mDaysAll')?.checked; state.monitor.days=allChecked?'all':(checked.length?checked:'all'); state.monitor.sort=document.getElementById('mSort')?.value||'desc'; state.monitor.search=document.getElementById('mSearch')?.value||'';}
function rememberMonitorMenuScroll(){const city=document.getElementById('mCityMenu'), days=document.getElementById('mDaysMenu'); if(city)monitorMenuScroll.city=city.scrollTop; if(days)monitorMenuScroll.days=days.scrollTop;}
...
async function copyText(txt){if(!txt)throw new Error('Sem BRs para copiar.'); if(navigator.clipboard&&navigator.clipboard.writeText){try{await navigator.clipboard.writeText(txt); return;}catch{}} if(!document.createElement||!document.body)throw new Error('Clipboard indisponível.'); const ta=document.createElement('textarea'); ta.value=txt; ta.setAttribute('readonly',''); ta.style.position='fixed'; ta.style.left='-9999px'; document.body.appendChild(ta); ta.select(); const ok=document.execCommand&&document.execCommand('copy'); document.body.removeChild(ta); if(!ok)throw new Error('Clipboard indisponível.');}
async function copyOpenBRs(){const rows=window.__modalRows||[], txt=rows.map(r=>r.shipment_id).filter(Boolean).join('\n'); try{await copyText(txt); setStatus(rows.length+' BR(s) copiadas do detalhe.','ok');}catch(err){console.error(err); setStatus('Nao consegui copiar automaticamente. Selecione e copie manualmente.','warn');}}
async function copyMonitorBRs(){const rows=monitorVisibleRows(state.rows.filter(r=>monitorMatch(r,state.monitor.type))).slice().sort((a,b)=>String(a.city).localeCompare(String(b.city),'pt-BR')||String(a.bairro).localeCompare(String(b.bairro),'pt-BR')||(a.ageing_num??9999)-(b.ageing_num??9999)||String(a.shipment_id).localeCompare(String(b.shipment_id))); const txt=rows.map(r=>r.shipment_id).filter(Boolean).join('\n'); try{await copyText(txt); setStatus(rows.length+' BR(s) copiadas de '+monitorLabel(state.monitor.type)+'.','ok');}catch(err){console.error(err); setStatus(rows.length?'Nao consegui copiar automaticamente. Abra um grupo ou tente novamente.':'Nenhuma BR encontrada para copiar nesta visão.','warn');}}
...
function currentRowsForExport(){if(state.view==='base')return filteredRows(); if(state.view.startsWith('monitor-'))return monitorVisibleRows(state.rows.filter(r=>monitorMatch(r,state.monitor.type))); return state.rows;}

async function hydrateCloudOnBoot(){try{await loadTreatmentsFromCloud(true); const loaded=await loadDashboardStateFromCloud(true); if(!loaded&&!state.rows.length)setStatus('Dashboard carregado. Importe a STUCKS do dia. A base importada ficará salva na nuvem.','warn'); else if(!loaded)setStatus('Dashboard carregado com a base local deste navegador.','warn');}catch(err){console.error(err); if(!state.rows.length)setStatus('Não consegui carregar a base da nuvem. Importe a STUCKS ou confira a tabela no Supabase.','error');}}
function buildCloudPayload(kind,fileName){return {version:1,kind:kind||'manual',savedAt:new Date().toISOString(),currentFile:state.currentFile||fileName||'',importedAt:state.importedAt,rows:state.rows,cepMap:[...state.cepMap],damages:[...state.damages],history:state.history};}
async function saveDashboardStateManual(){if(!state.rows.length)return setStatus('Não há base carregada para salvar na nuvem.','warn'); saveLocal(true); await saveDashboardStateToCloud('manual','manual'); setStatus('Base atual salva neste navegador e na nuvem.','ok');}
async function saveDashboardStateToCloud(kind,fileName){if(!currentSession)throw new Error('Login não encontrado para salvar a base na nuvem.'); const payload=buildCloudPayload(kind,fileName); const rec={id:CLOUD_STATE_ID,payload,row_count:state.rows.length,cep_count:state.cepMap.size,current_file:state.currentFile||fileName||'',imported_at:state.importedAt,updated_by:null,updated_by_email:currentSession.user?.email||'',updated_at:new Date().toISOString()}; await supabase(CLOUD_STATE_TABLE+'?on_conflict=id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(rec)}); return rec;}
async function loadDashboardStateFromCloud(silent){if(!currentSession)return false; try{const rows=await supabase(CLOUD_STATE_TABLE+'?select=payload,row_count,cep_count,current_file,imported_at,updated_at,updated_by_email&id=eq.'+encodeURIComponent(CLOUD_STATE_ID)+'&limit=1'); const data=Array.isArray(rows)?rows[0]:null; const payload=data?.payload; if(!payload||!Array.isArray(payload.rows)||!payload.rows.length)return false; applyCloudPayload(payload); const when=data.updated_at?formatDate(data.updated_at):'-'; setStatus('Base carregada da nuvem: '+state.rows.length+' stucks. Última atualização: '+when+'.','ok'); return true;}catch(error){console.warn('loadDashboardStateFromCloud',error); const msg=String(error.message||error).includes('PGRST205')||String(error.message||error).includes('Could not find the table')?'Tabela de base na nuvem ainda não existe no Supabase STUCKS TRATATIVAS ou a API ainda não recarregou.':'Erro ao carregar base da nuvem: '+(error.message||error); if(!silent||String(error.message||error).includes('Could not find the table'))setStatus(msg,'warn'); return false;}}
function applyCloudPayload(payload){state.rows=Array.isArray(payload.rows)?payload.rows:[]; state.cepMap=new Map(Array.isArray(payload.cepMap)?payload.cepMap:[]); state.damages=new Set(Array.isArray(payload.damages)?payload.damages:[]); state.history=Array.isArray(payload.history)?payload.history:[]; state.currentFile=payload.currentFile||'Base salva na nuvem'; state.importedAt=payload.importedAt||payload.savedAt||null; state.rows.forEach(enrich); applyCep(); applyDamages(); applyTreatments(); resetFilters(); saveCep(); saveDamages(); saveLocal(true); render();}

async function loadTreatmentsFromCloud(silent){try{const rows=await supabase('stucks_tratativas?select=shipment_id,tratativa,tracking_status,cidade,bairro,driver,ageing_last_status,avaria,updated_at&order=updated_at.desc&limit=10000'); let count=0, cloud=new Map(); (rows||[]).forEach(r=>{const br=normalizeTrace(r.shipment_id),t=normalizeTreatmentSave(r.tratativa); if(br&&t){cloud.set(br,t);count++;}}); state.treatments=cloud; state.rows.forEach(r=>{if(!cloud.has(r.shipment_id))r.tratativa='';}); applyTreatments(); saveLocal(true); renderBadges(); if(state.rows.length) render(); if(!silent)setStatus('Tratativas importadas da nuvem: '+count+' BR(s). Agora importe a STUCKS para cruzar pela BR.','ok'); return count;}catch(err){console.error(err); if(!silent)setStatus('Erro ao puxar Supabase: '+(err.message||err),'error'); return 0;}}
async function syncAllTreatments(){const rec=[...state.treatments.entries()].map(([br,t])=>buildTreatmentRecord(br,t)).filter(r=>r.tratativa); await upsertTreatments(rec); setStatus('Sincronização concluída: '+rec.length+' tratativa(s).','ok');}
...
function buildTreatmentRecord(br,t){const r=state.rows.find(x=>x.shipment_id===normalizeTrace(br)); return {shipment_id:normalizeTrace(br),tratativa:normalizeTreatmentSave(t),tracking_status:r?.tracking_status||'',cidade:r?.city||'',bairro:r?.bairro||'',driver:r?.driver||'',ageing_last_status:r?.ageing_last_status||'',avaria:r?.avaria||'Não'};}
...
function clearBase(){if(!confirm('Limpar a base atual?'))return; state.rows=[]; state.filtered=[]; state.currentFile=''; state.importedAt=null; saveLocal(true); render(); setStatus('Base limpa neste navegador. A base da nuvem permanece salva até uma nova importação.','warn');}
...
function normalizeKeyword(v){return normalizeYesNo(v)||clean(v);}
function normalizeImportedTreatment(v){const t=clean(v); return normalizeYesNo(t)?'':t;}
